        ...
        {expanded && (
          ...
          >
            <Droppable droppableId={`sidebar-${project.id}`}>
              {(provided, snapshot) => (
                <div ref={provided.innerRef} {...provided.droppableProps} className={`pb-2 ml-3 pl-3 border-l-2 transition-colors ${snapshot.isDraggingOver ? "border-indigo-300 dark:border-indigo-600 bg-indigo-50/50 dark:bg-indigo-900/10 rounded-md" : "border-gray-100 dark:border-gray-800"}`}>
                  {tasks.length === 0 ? (
                    ...
                    tasks.map((task, index) => (
                      <Draggable key={task.id} draggableId={`sidebar-${task.id}`} index={index}>
                        {(drag, snapshot) => (
                          ...
                          >
                            <div {...drag.dragHandleProps} className="flex-shrink-0 cursor-grab opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <button className="flex-shrink-0 w-5 h-5 flex items-center justify-center opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity rounded hover:bg-gray-100 dark:hover:bg-gray-700">
